 ...
 * 作业：为Pet类书写一个Comparator，按照自己喜爱的宠物程度将其排序，并用 容器存放它们
 * 可参照最下面的MyComparator
...
class myFaviourtPet{
    public static void main(String[] args) {
        ...
        PriorityQueue queue = new PriorityQueue(4,sort);
        queue.add(new Pet("小猫",4));
        queue.add(new Pet("小狗",3));
        queue.add(new Pet("小鸟",2));
        queue.add(new Pet("小猪",1));
       while(true){
           Pet pet = (Pet) queue.poll();
           if(pet != null){
...
class ComparatorSort implements Comparator<Pet>{
